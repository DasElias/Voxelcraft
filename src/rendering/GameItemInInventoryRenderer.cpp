...
namespace vc {
	void GameItemInInventoryRenderer::render(std::shared_ptr<GameItem> item, const TextureArray& blockTextures, float absXMargin, float absYMargin, float width, float height, float opacity) {
		...
		if(item->isBlock()) {
			...
			blockRenderer.render(derived, blockTextures, absXMargin, absYMargin, width, height, opacity);
		} else {
